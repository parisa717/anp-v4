import { TransformedGraphQLError } from '../sharedTypes'
...
const concatErrorCache = <T, ID>(
  existingCache: CacheList<T, ID>,
  error: TransformedGraphQLError | undefined,
): CacheList<T, ID> => {
 ...
 */
type InnerProvidesList<T> = <Results extends { id: unknown }[], Error extends TransformedGraphQLError>(
...
type InnerProvidesListWithCustomId<T> = <
  Results extends Record<string, unknown>[],
  Error extends TransformedGraphQLError,
>(
  results: Results | undefined,
  error: Error | undefined,
) => CacheList<T, string>
...
type InnerProvidesNestedList<T> = <Results extends { data: { id: unknown }[] }, Error extends TransformedGraphQLError>(
  results: Results | undefined,
  error: Error | undefined,
